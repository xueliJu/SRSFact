...
class GPTModel(Model):
    ...
    def _get_encoding(cls):
        ...
        if cls._encoding_load_failed:
            return None

        try:
            cls._encoding = tiktoken.get_encoding("cl100k_base")
        except Exception:
            cls._encoding_load_failed = True
            return None
        return cls._encoding

    def _estimate_tokens(self, text: str) -> int:
        # Fallback for offline / restricted-network environments where tiktoken
        # cannot download the cl100k_base vocabulary on first use.
    ...
    def count_tokens(self, prompt: Prompt | str) -> int:
        ...
        if not getattr(self, "_warned_about_tiktoken_fallback", False) and self.logger is not None:
            self.logger.warning(
                "Failed to load tiktoken encoding cl100k_base. "
                "Falling back to an approximate token counter. "
                "If you need exact token counts, pre-cache the file "
                "https://openaipublic.blob.core.windows.net/encodings/cl100k_base.tiktoken "
                "via TIKTOKEN_CACHE_DIR."
            )
            self._warned_about_tiktoken_fallback = True
